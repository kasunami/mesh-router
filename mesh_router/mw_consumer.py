...
def process_mw_message(
    ...
    *,
    ...
    with _mw_state_db_connect() as conn:
        with conn.cursor() as cur:
            if message_type == "heartbeat":
                ...
                _upsert_mw_host(
                    cur,
                    host_id=host_id,
                    actual_profile=str(inner.get("actual_profile") or "") or None,
                ...
                # Ensure stub service rows exist for all referenced lanes
...
def _upsert_mw_host(
    ...
    *,
    ...
    cur.execute(
        ...
        """,
        (
            host_id, host_id, "unknown", "unknown", 1,
            actual_profile, actual_profile,
            last_heartbeat_at, last_state_at,
    ...
    for l in lane_states:
        ...
        cur.execute(
            ...
            ON CONFLICT (host_id, lane_id) DO UPDATE SET
              ...
              last_loaded_model = EXCLUDED.last_loaded_model,
              desired_state = EXCLUDED.desired_state,
              actual_state = EXCLUDED.actual_state,
              health_status = EXCLUDED.health_status,
            ...
            (
                ...
                str(l.get("lane_type") or "other"),
                str(l.get("backend_type") or "unknown"),
                service_id,
                str(l.get("resource_class") or ""),
                str(l.get("desired_model") or ""),
                str(l.get("actual_model") or ""),
                str(l.get("last_loaded_model") or ""),
                desired_state,
                actual_state,
                health_status,
                last_healthy_at,
...
def _upsert_transition(cur: Any, *, request_id: str, host_id: str, payload: dict[str, Any], observed_at: datetime) -> None:
    ...
    cur.execute(
        ...
        INSERT INTO mw_transitions (
          request_id, host_id, status, ok, error_kind, error_message,
        ...
        (
            ...
            ok,
            str(payload.get("error", {}).get("code") or "") or None,
            str(payload.get("error", {}).get("message") or "") or None,
            observed_at,
            observed_at if str(payload.get("status")) == "completed" else None,
        ),
    )
...
def _insert_transition_event(cur: Any, *, request_id: str, host_id: str, payload: dict[str, Any], observed_at: datetime) -> None:
    cur.execute(
        """
        INSERT INTO mw_transition_events (request_id, host_id, event_type, payload, created_at)
        VALUES (%s, %s, %s, %s, %s)
        """,
        (request_id, host_id, str(payload.get("event_type") or "status"), Jsonb(payload), observed_at),
    )


def _response_host_state_snapshot(payload: dict[str, Any]) -> dict[str, Any] | None:
    if "actual_models" in payload or "lane_states" in payload:
        return payload
    return None
    ...
    try:
        while True:
            ...
            try:
                ...
                observed_at = datetime.fromtimestamp(msg.timestamp()[1] / 1000.0, UTC)
                process_mw_message(payload, observed_at=observed_at, runtime_store=runtime_store)
                consumer.commit(message=msg, asynchronous=False)
            except Exception as e:
                logger.error("Failed to process MW message: %s", e, exc_info=True)
    finally:
        consumer.close()
