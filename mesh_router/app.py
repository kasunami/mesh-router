...
def _path_model_request_matches_candidate(
    requested_model_name: str,
    candidate_model_name: str,
    candidate_tags: list[str] | None = None,
) -> bool:
    requested = (requested_model_name or "").strip()
    candidate = (candidate_model_name or "").strip()
    ...
    return bool(requested_keys & set(_normalized_model_tags(candidate_tags)))
...
def _model_request_matches_candidate(
    ...
    if _is_path_model_request(requested_model_name):
        return _path_model_request_matches_candidate(
...
def _set_lane_suspension(cur, *, lane_id: str, suspended: bool, reason: str) -> None:
    ...
    cur.execute(
        ...
        UPDATE lanes
        SET status=CASE
              WHEN %s AND status IN ('offline', 'suspended', 'error')
                THEN 'ready'
              WHEN status IN ('offline', 'suspended', 'error')
                   AND COALESCE(suspension_reason, '')=%s
                THEN 'ready'
              ELSE status
            END,
        ...
        WHERE lane_id=%s
        """,
        (clear_unreasoned, reason, clear_unreasoned, reason, lane_id),
    )
    ...
    *,
    ...
    try:
        ...
        for _acquire_attempt in range(3):
            ...
            if choice and not is_llama_router_lane and not _model_request_matches_candidate(target_model_name, choice.current_model_name or ""):
        ...
        out.append(
            {
                ...
                "lane_id": lane_id,
                "lane_name": str(r.get("lane_name") or ""),
                ...
                "proxy_auth_metadata": r.get("proxy_auth_metadata") if isinstance(r.get("proxy_auth_metadata"), dict) else None,
